...
class FilterRepository
{
    ...
    public function getAllActiveProductt($sort_by, $paginate,$slug)
    {
        if($slug){
            $products = $this->getSectionProducts($slug)['products'];
        }else{
            $products = SellerProduct::with('skus', 'product')->where('status', 1)->whereHas('product', function ($query) {
                $query->where('status', 1);
            })->activeSeller();
...
//            dd($column,$sort);
        ...
        return $products->paginate(($paginate_by != null) ? $paginate_by : 16);
    ...
    public function getConvertedMin($value){
        ...
        if(auth()->check() && auth()->user()->currency_code != app('general_setting')->currency_code){
