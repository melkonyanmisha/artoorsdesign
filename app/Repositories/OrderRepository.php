...
//            $customer_phone = $checkoutRepo->activeShippingAddress()->phone;
        ...
        $order = Order::create([
            ...
            'customer_phone' => '',
            'customer_shipping_address' => $shipping_address?@$shipping_address->id:null,
            'customer_billing_address' => $billing_address?@$billing_address->id:null,
...
//            $fileName =  uniqid().'Artoors.zip';
            foreach($productList as $key => $product){
                if(auth()->check()){
                    \App\Models\Paymant_products::create([
                        'user_id' => auth()->id(),
                        'product_id' => $product->product->product->id,//seller_products
                        'product_id1' => $product->product->product->product->id,//products
//                        'ip_addr' => request()->ip()
                    ]);
                }else{
                    \App\Models\Paymant_products::create([
...
//                $link = route('artoors.files',$product->product->product->product->id);
//                $password = $product->product->product->product->password;
//
//                Mail::to($customer_email)->send(new SendMail('ArtoorsDesign',$link,$password));

//                if($pdf = $product->product->product->product->pdf){
//                    if ($zip->open(public_path($fileName), ZipArchive::CREATE) === TRUE) {
//                        $relativeNameInZipFile = basename(public_path($pdf));
//                        $zip->addFile(public_path($pdf), $relativeNameInZipFile);
//                    }
////                    $path = explode('/',$pdf);
...
//            }
        ...
        \session()->put('arcaMessage',123465);
//        Mail::to($customer_email)->send(new \App\Mail\OrderMail($order));

//        session()->put('arcaMessage','1345');
