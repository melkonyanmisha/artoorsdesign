...
//                    'postal_code' => $data['postal_code'],
                ...
                CustomerAddress::where('id',$data['address_id'])->first()->update([
                    'name' => $data['name'],
                    'email' => $data['email'],
                    'lastname' => $data['lastname'],
//                    'phone' => $data['phone'],
...
//            $address['postal_code'] = $data['postal_code'];
    ...
    public function activeShippingAddress(){
        if(auth()->check()){
            ...
            $address = session()->get('shipping_address')?(object) session()->get('shipping_address'):null;
        }
        return $address;
    }
    ...
    public function activeBillingAddress(){
        $billingAddress = null;
        if(auth()->check()){
            $billingAddress = auth()->user()->customerAddresses->where('is_billing_default',1)->first();
