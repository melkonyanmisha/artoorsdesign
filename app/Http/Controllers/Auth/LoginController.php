...
//            return $this->loginDone($request, $user);
    ...
    public function loginDone($request, $user = null){
        ...
        if($user){
            $this->guard()->login($user,$request->get('remember'));
            $loged_in = true;
        }else{
...
//        $check_user = $this->guard()->getLastAttempted();

        \auth()->user()->active_user = '0';
        \auth()->user()->when_not_active = Carbon::now();
        \auth()->user()->save();
