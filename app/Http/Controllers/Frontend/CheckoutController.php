        ...
        if($step == 'select_shipping'){
            ...
            if(auth()->check()){
                ...
                if($request->get('address_id') != 0){
                    $this->checkoutService->addressUpdate($request->only('address_id','name','email','country'));
                }else{
                    $this->checkoutService->addressStore($request->only('name','email','country'));
                }

            }else{
                $this->checkoutService->guestAddressStore($request->only('name','email','country'));
            }
            ...
            return view(theme('new.payment'),$data);
...
//            return view(theme('new.payment'),$infoCompleteOrder);
...
//        if($step == 'complete_order'){
            ...
            $infoCompleteOrder['order_payment_id'] = $request->get('payment_id');
            $infoCompleteOrder['order_gateway_id'] = $request->get('gateway_id');
            ...
            $orderData = [
                ...
                'delivery_date' => $delivery_date,
                'order_payment_id' => $request->get('payment_id'),
                'payment_method' => $request->get('gateway_id'),
            ...
            if(auth()->check()){
                ...
                return redirect()->route('frontend.customer_profile',['a' => 'purchases'])->with('arcaMessage', 'arca.success');
            }
